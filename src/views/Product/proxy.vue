  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="最小金额" width="120">
        <template slot-scope="scope">{{ scope.row.account_min }}</template>
      </el-table-column>
      <el-table-column align="center" label="最大金额" width="120">
        <template slot-scope="scope">{{ scope.row.account_max }}</template>
      </el-table-column>
      ...
      <el-table-column align="center" label="是否可用" width="120">
        <template slot-scope="scope">
          <input
            type="checkbox"
            :checked="scope.row.isenable==0 ? false: true"
            v-on:onclick="enable(scope.row.isenable)"
      ...
      <el-form>
        ...
        <el-row class="row">
          <el-col :span="8">
            ...
            <input
              type="checkbox"
              :checked="form.isenable==0 ? false: true"
        ...
        <el-row class="row">
          <el-upload
            action="http://api.rongxiaoxiao.com/uploadfile"
        ...
        <el-row class="row">
          <el-col :span="8">
            <img v-bind:src="form.image" style="width:100%">
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row class="row">
          <el-upload
            action="http://api.rongxiaoxiao.com/uploadfile"
            :on-success="handledescriptionSuccess"
            :on-preview="handlePictureCardPreview"
            :on-remove="handleRemove"
            :show-file-list="false"
            :on-exceed="exceed"
          >
            <el-button type="primary">产品宣传页</el-button>
          </el-upload>
        </el-row>
        <el-row class="row">
          <el-col :span="8">
            <img v-bind:src="form.description" style="width:100%">
          </el-col>
        </el-row>
        <el-row>
          <tinymce ref="editor" :height="400" v-model="editContent"/>
        ...
        <el-row>
          ...
          >
            ...
            <el-table-column align="center" label="状态" width="100">
              <template slot-scope="scope">{{ scope.row.isEnable==1?'可用':'禁用'}}</template>
            </el-table-column>
            <el-table-column align="center" label width="100">
              <template slot-scope="scope">
                <el-button
                  ...
                  class="filter-item"
                  type="primary"
                  @click="updatetypestatus(scope.row.productTypeid,0)"
                >禁用</el-button>
                <el-button
                  v-else
                  class="filter-item"
                  type="primary"
                  @click="updatetypestatus(scope.row.productTypeid,1)"
                >启用</el-button>
              </template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      imgDataUrl: "",
      uploadurl: "http://127.0.0.1:80/uploadfile",
      busnessTypes: [],
      busnessType: "",
      typelist: [],
      selectProductID: -1,
  ...
  methods: {
    ...
    handledescriptionSuccess(res, file) {
      console.log("uploadDone");
      console.log(res.data);
    ...
    updatetypestatus(producttypeid, status) {
      let self = this;
      updatetypestatus({ productTypeid: producttypeid, IsEnable: status }).then(
        response => {
          this.getProducttype();
        }
      );
